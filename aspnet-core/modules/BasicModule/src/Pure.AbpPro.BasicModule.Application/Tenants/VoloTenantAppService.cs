...
namespace Pure.AbpPro.BasicModule.Tenants
{
    ...
    public class VoloTenantAppService : BasicModuleAppService, IVoloTenantAppService
    {
        ...
        { 
            ...
            if (tenant == null)
            {
                throw new BusinessException(BasicModuleErrorCodes.TenantNotExist);
            }
        ...
        public async Task AddOrUpdateConnectionStringAsync(AddOrUpdateConnectionStringInput input)
        {
            ...
            {
                throw new BusinessException(BasicModuleErrorCodes.NotSupportSetConnectionString);
            }
            var tenant = await _tenantRepository.FindAsync(input.Id, true);
