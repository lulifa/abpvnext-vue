...
{
    public class AccountAppService : BasicModuleAppService, IAccountAppService
    {
        ...
        public virtual async Task<LoginOutput> LoginAsync(LoginInput input)
        {
            ...
            {
                throw new BusinessException(BasicModuleErrorCodes.UserDisabled);
            }

            if (result.IsLockedOut)
            {
                throw new BusinessException(BasicModuleErrorCodes.UserLockedOut);
            }

            if (!result.Succeeded)
            {
                throw new BusinessException(BasicModuleErrorCodes.UserOrPasswordMismatch);
            }
        ...
        {
            if (!user.IsActive) throw new BusinessException(BasicModuleErrorCodes.UserLockedOut);
