...
{
    [Authorize(policy: BasicModulePermissions.SystemManagement.Setting)]
    public class SettingAppService : BasicModuleAppService, ISettingAppService
    {
        ...
        public virtual async Task<List<SettingOutput>> GetAsync()
        {
            ...
            var settings = allSettings
                .Where(e => e.Properties.ContainsKey(BasicModuleSettings.Group.Default)).ToList();

            var settingOutput = settings
                .GroupBy(e => e.Properties[BasicModuleSettings.Group.Default].ToString()).Select(s =>
                    new SettingOutput
                    {
                        Group = s.Key,
